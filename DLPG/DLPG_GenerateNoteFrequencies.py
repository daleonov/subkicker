...
def GenerateHeader(lsNoteLabels, dNoteFrequencies, sFileName):
	...
	nNoteNamesInLine = 12
	# Header's header
	sGeneratedFreqsMacro = 'DLPG_GENERATED_FREQS_NUMBER'
	...
	sHeader += '#define ' + sHeaderMacro + sNewLine
	sHeader += '#define ' + sGeneratedFreqsMacro + ' ' + str(len(lsNoteLabels)) + sNewLine*2
	# Frequencies
	sHeader += 'double afNoteFrequencies[' + sGeneratedFreqsMacro + '] = {' + sNewLine + sIndentation
	...
	nIndex = 1
	# Note labels
	sHeader += 'char* asNoteLabels[] = {' + sNewLine + sIndentation
	for sNoteLabel in lsNoteLabels:
